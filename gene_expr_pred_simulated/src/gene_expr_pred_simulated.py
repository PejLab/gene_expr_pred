...
#            tqdm.write(str(var_count)+"-eQTL genes", end = "\r")
            columns = pd.Series();
            columns = columns._append(pd.Series({'gene_id':row['gene_id']}))
            ...
            if ('chrX' in row['variant_id_1']):# chrX won't be found in vcf file
                for chrx_count in range(1,var_count+1):
                    ...
                    columns = columns._append(pd.Series({var:row[var]}))
...
#            print (count, end='\r')
            ...
            prediction_matrix_haplotype1= prediction_matrix_haplotype1._append(temp,ignore_index=True);
            prediction_matrix_haplotype2= prediction_matrix_haplotype2._append(temp,ignore_index=True);
...
# predicting ASE and total expression     
    ...
    def ASE_predicted_value(hap1_count,hap2_count, total_exp):
        h1 = 2**hap1_count
        htotal = 2**hap1_count + 2**hap2_count
        p = h1 / htotal
        h1_noise = np.round(np.random.binomial(int(total_exp), p ,1),2)
        h2_noise = np.round(int(total_exp) - h1_noise,2)
        
        
        s = str(h1_noise)+"|"+str(h2_noise)
        
        return s

    def total_predicted_value(e_init,hap1_count,hap2_count):
        hap1_count = (e_init * 2**hap1_count)      
        hap2_count = (e_init * 2**hap2_count) 
        total_expression = hap1_count + hap2_count
        Total_expression_noise = np.random.poisson(int(total_expression),1) 
    

        return (Total_expression_noise);
   
    
    e0_output = pd.DataFrame(columns=['gene_id','e0'])
    for var_count in range(1,max_var+1):
...
#            print (var_count, "   ", count, end='\r')
            ###make e0
            e0 = 10**np.random.normal(2, 0.5, 1)
            e0_output = e0_output._append({'gene_id':row['gene_id'],'e0':e0},ignore_index=True)
            
        
            #df_extended = pd.DataFrame(e0_output_lst, columns=['gene_id', 'e0'])
           
            #df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
           
            #e0_output= pd.concat([e0_output,df_extended],ignore_index=True)
            ...
            for ind in individuals:
                ...
                if (not '2' in str(row[ind]) and not '2' in str(genotype_haplotype2.loc[index,ind])):
                    ...
                    total_prediction = total_predicted_value(e0,lookup_Table.loc[lookup_Table_index,x1],lookup_Table.loc[lookup_Table_index,x2])
                    total_count_lst.insert(ind_index,np.round(total_prediction,4))
                    
                    ASE_prediction = ASE_predicted_value(lookup_Table.loc[lookup_Table_index,x1],lookup_Table.loc[lookup_Table_index,x2],total_prediction)
    ...
    e0_output.to_csv(os.path.expanduser(output_path+'/e0.txt'),index = False, sep ='\t')
...
if __name__ == "__main__":
    ...
    # REQUIRED
    parser.add_argument("--input", required=True, help="input containing gene and variant IDs")
    ...
    parser.add_argument("--mean", default = 0, required=False, help="mean for aFC distribution")
    parser.add_argument("--sd", default =1, required=False, help="sd for aFC distribution")
...
#    logging.info("Run starting at " + str(datetime.datetime.now))
    ...
    if (args.chr == None):
        bashCommand='Rscript '+script_path+'/gene_expression_lookupTable_simulated.R '+ aFC_path + " "+path_lookup+" "+sep+" "+str(variant_max)+" "+str(mean_val)+" "+str(sd_val)
    else:
        bashCommand='Rscript '+script_path+'/gene_expression_lookupTable_simulated.R '+ aFC_path + " "+path_lookup+" "+sep+" "+str(variant_max)+" "+args.chr+" "+str(mean_val)+" "+str(sd_val)
    os.system(bashCommand)
